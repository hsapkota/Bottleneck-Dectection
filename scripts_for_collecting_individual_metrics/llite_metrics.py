...
class fileThread(threading.Thread):
    ...
    def run(self):
        out_file = open("llite_"+metric+".txt", "a+")
        while True:
...
for name in stat_file_names:
    if "llite" in name:
        proc = Popen(['ls', '-l', '/proc/fs/lustre/llite'], universal_newlines=True, stdout=PIPE)
        res = proc.communicate()[0]
        parts = res.split("\n")
        for x in range(1, len(parts)):
            name_parts = parts[x].split(" ")
            if "panda" in name_parts[len(name_parts) - 1]:
                llite_path.append('/proc/fs/lustre/llite/' + name_parts[len(name_parts) - 1])

        metric_path = llite_path[0]
        ...
        for x in range(1, len(parts)):
            ...
            if not str(metrics_name_parts[0]).startswith('d'):
                llite_metrics.append(metrics_name_parts[len(metrics_name_parts) - 1])
                # print(metrics_name_parts[len(metrics_name_parts) - 1])

i = 0
index_number = 0
for llite in llite_path:
    for metric in llite_metrics:
...
#         for metric in llite_metrics:
#             if len(metric) >= 1:
#                 if "dump_page_cache" not in metric:
#                     metric_path = llite+"/"+metric
