...
public class BackgroundSender {
    ...
    // public BackgroundSender(String host, int port) {
    //     try {
    ...
    public static class SendThread extends Thread {
        ...
        public SendThread(String host, int port, String path, int label){
            this.host = host;
            this.port = port;
            this.path = path;
            this.label = label;
        ...
        public void run() {
            try {
                ...
                sendFile(this.path, this.label, this.s);
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    public static void sendFile(String path, int label, Socket s) throws IOException {

        boolean allFileTransfersCompleted = false;
        
        List<FiverFile> files;
        LinkedBlockingQueue<FiverFile> checksumFiles;

        long totalTransferredBytes = 0;
        long totalChecksumBytes = 0;
        long INTEGRITY_VERIFICATION_BLOCK_SIZE = 256 * 1024 * 1024;
        long startTime;
        boolean debug = false;
    ...
    public static void main(String[] args) {
        ...
        while(true){
            ...
            for (String filename : files) {
                try{
                    SendThread sendThread = new SendThread(destIp, port, path+filename, label);
                    sendThread.start();
